...
serve(async (req: Request) => {
  try {
    ...
    const { data: booking, error } = await supabaseAdmin
      .from("bookings")
      .select("id, check_in, base_price, payment_intent_id, status, tenant_id")
      .eq("id", bookingId)
      .single();
    ...
    // Notify tenant their booking has been cancelled
    fetch(`${supabaseUrl}/functions/v1/send-push-notification`, {
      ...
      body: JSON.stringify({
        ...
        body: `Your booking for check-in on ${booking.check_in} has been cancelled. A refund of £${refundAmount / 100} is on its way.`,
        data: {
          screen: "my-bookings",
    ...
    return new Response(
      JSON.stringify({ ok: true, refundAmount: refundAmount / 100 }),
      {
