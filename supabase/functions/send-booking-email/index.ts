...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!supabaseUrl || !serviceRoleKey || !emailLinkSecret) {
      throw new Error(
        "Missing SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, or EMAIL_LINK_SECRET",
      );
    }
    ...
    if (landlordError || !landlord?.email)
      throw new Error("Landlord email missing");
    ...
    const bodyText = `Hi ${landlord.first_name ?? "there"},
