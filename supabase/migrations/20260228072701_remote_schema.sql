...
CREATE POLICY "Admins can view waitlist" ON "public"."waitlist" FOR SELECT USING (("auth"."role"() = 'authenticated'::"text"));



CREATE POLICY "Allow read access to amenities for authenticated users" ON "public"."amenities" FOR SELECT TO "authenticated" USING (true);



CREATE POLICY "Allow read access to property_amenities for authenticated users" ON "public"."property_amenities" FOR SELECT TO "authenticated" USING (true);
...
CREATE POLICY "Anyone can view user profile" ON "public"."profiles" FOR SELECT TO "authenticated" USING (true);
...
CREATE POLICY "Enable users to view their own data only" ON "public"."notifications" TO "authenticated" USING ((( SELECT "auth"."uid"() AS "uid") = "user_id"));



CREATE POLICY "Landlords can manage amenities for their own properties" ON "public"."property_amenities" TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."properties"
  WHERE (("properties"."id" = "property_amenities"."property_id") AND ("properties"."landlord_id" = "auth"."uid"()))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."properties"
...
CREATE POLICY "Landlords can update status of bookings for their properties" ON "public"."bookings" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."properties"
  WHERE (("properties"."id" = "bookings"."property_id") AND ("properties"."landlord_id" = "auth"."uid"())))));



CREATE POLICY "Landlords can view and update bookings for their properties" ON "public"."bookings" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."properties"
  WHERE (("properties"."id" = "bookings"."property_id") AND ("properties"."landlord_id" = "auth"."uid"())))));
...
CREATE POLICY "Policy with table joins" ON "public"."profiles" FOR UPDATE USING (( SELECT ("auth"."uid"() = "profiles"."id")));



CREATE POLICY "Tenants can view and manage their own bookings" ON "public"."bookings" TO "authenticated" USING (("tenant_id" = "auth"."uid"())) WITH CHECK (("tenant_id" = "auth"."uid"()));
...
CREATE POLICY "Users can view payments related to their bookings" ON "public"."payments" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
