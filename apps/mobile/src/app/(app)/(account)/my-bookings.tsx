...
export default function MyBookingsScreen() {
  ...
  const upcomingBookings = bookings.filter((b) => {
    ...
    const isFutureBooking = new Date(b.check_out) >= now;
    return isActiveStatus && isFutureBooking;
  });

  const pastBookings = bookings.filter((b) => {
    const isPastStatus = b.status === "cancelled" || b.status === "completed";
    const isHistoricalBooking = new Date(b.check_out) < now;
    return isPastStatus || (b.status === "confirmed" && isHistoricalBooking);
  });
  ...
  const renderBookingCard = ({ item }: { item: BookingWithProperty }) => {
    ...
    const isPast = new Date(item.check_in) <= now;
